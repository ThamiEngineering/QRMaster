...
<template>
  <div class="bg-white border border-gray-200 rounded-3xl p-6 h-full overflow-y-auto">
      ...
      <div v-if="isOfflineMode" class="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
        <div class="flex items-center">
          ...
          <p class="text-sm text-yellow-800">
            Mode hors ligne : Connexion limitée. Certaines fonctionnalités peuvent être
            indisponibles.
          </p>
        </div>
      </div>
    ...
    <div v-if="isLoading" class="text-center py-8">
      <div
        class="animate-spin w-8 h-8 border-4 border-yellow-500 border-t-transparent rounded-full mx-auto"
      />
      ...
      <div class="bg-gray-50 rounded-xl p-6">
        ...
        <form v-else class="space-y-4" @submit.prevent="saveProfile">
          ...
          <div class="flex space-x-3 pt-4">
            ...
            <UButton variant="outline" icon="i-heroicons-x-mark" @click="cancelEditing">
              Annuler
            </UButton>
          </div>
        </form>
