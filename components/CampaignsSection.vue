...
<template>
  <div class="space-y-8">
    <div v-if="campaigns.length > 0" class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
      <div
        class="group relative p-6 rounded-2xl overflow-hidden bg-gradient-to-br from-white/[0.03] via-white/[0.05] to-white/[0.03] backdrop-blur-md border border-white/10 hover:border-white/20 transition-all duration-300"
      >
        <div
                  ...
                  campaigns.reduce(
                    (sum, campaign) =>
                      ...
                      qrcodes
                        ...
                        .reduce((qrSum, qr) => qrSum + (qr.scan_count || 0), 0),
                    0
                  )
                }}
              </p>
          ...
          <div class="flex items-center gap-4 mb-4">
            ...
            <div>
              <p class="text-2xl font-bold text-white">
                {{
                  campaigns.reduce(
                    (sum, campaign) => sum + getQRCodeCountForCampaign(campaign.id),
                    0
                  )
                }}
              </p>
    ...
    <div v-if="loadingCampaigns" class="flex items-center justify-center py-20">
      <div class="flex items-center gap-4">
        <div
          class="animate-spin rounded-full h-8 w-8 border-2 border-green-400 border-t-transparent"
        />
    ...
    <div v-else-if="campaigns.length === 0" class="flex flex-col items-center justify-center py-20">
      <div class="relative mb-8">
        <div
          class="w-24 h-24 bg-green-400/10 border border-green-400/20 rounded-2xl flex items-center justify-center"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="40"
      ...
      <p class="text-white/60 mb-8 text-center max-w-md">
        Créez votre première campagne pour organiser et structurer vos QR codes par thématique ou
        objectif.
      </p>
      <div
        class="p-4 rounded-xl bg-gradient-to-r from-green-400/5 to-emerald-400/5 border border-green-400/20 max-w-md"
      >
        <p class="text-sm text-green-400 text-center">
          💡 Utilisez le bouton "Nouvelle campagne" dans le header pour commencer
        </p>
      </div>
    </div>
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div
        ...
        :key="campaign.id"
        class="group relative rounded-2xl overflow-hidden bg-gradient-to-br from-white/[0.03] via-white/[0.05] to-white/[0.03] backdrop-blur-md border border-white/10 hover:border-white/20 shadow-lg hover:shadow-white/10 transition-all duration-500 ease-out"
      >
        <div
          class="absolute inset-0 bg-gradient-to-br opacity-0 group-hover:opacity-20 transition-opacity duration-500"
          :class="{
        ...
        <div class="relative p-6 flex flex-col h-full">
          <div class="flex items-start justify-between mb-6">
            <div class="flex-1">
              <h3
                class="font-bold text-xl text-white group-hover:text-green-400 transition-colors duration-300 line-clamp-2 mb-2"
              >
                {{ campaign.name }}
              </h3>
            ...
            <span
              class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ml-4 flex-shrink-0"
              :class="{
          ...
          <div class="flex-grow space-y-4">
            <div class="space-y-3 text-sm text-white/60">
              <div v-if="campaign.start_date" class="flex items-center gap-3">
                <div
                  class="w-5 h-5 rounded-lg bg-blue-400/10 border border-blue-400/20 flex items-center justify-center"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="12"
              ...
              <div v-if="campaign.end_date" class="flex items-center gap-3">
                <div
                  class="w-5 h-5 rounded-lg bg-red-400/10 border border-red-400/20 flex items-center justify-center"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="12"
              ...
              <div class="flex items-center gap-3">
                <div
                  class="w-5 h-5 rounded-lg bg-yellow-400/10 border border-yellow-400/20 flex items-center justify-center"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="12"
                ...
                <span class="font-medium text-white/90"
                  >{{ getQRCodeCountForCampaign(campaign.id) }} QR code{{
                    getQRCodeCountForCampaign(campaign.id) > 1 ? 's' : ''
                  }}</span
                >
              </div>
            </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="text-center">
                  <p class="text-2xl font-bold text-white">
                    {{
                      qrcodes.filter(
                        (qr) => qr.campaign_id === campaign.id && qr.type === 'dynamique'
                      ).length
                    }}
                  </p>
                ...
                <div class="text-center">
                  <p class="text-2xl font-bold text-white">
                    {{
                      getQRCodeCountForCampaign(campaign.id) > 0
                        ? Math.round(
                            qrcodes
                              .filter((qr) => qr.campaign_id === campaign.id)
                              .reduce((sum, qr) => sum + (qr.scan_count || 0), 0) /
                              getQRCodeCountForCampaign(campaign.id)
                          )
                        : 0
                    }}
                  </p>
          ...
          <div
            class="flex gap-2 mt-6 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0"
          >
            <button
              class="flex-1 relative overflow-hidden bg-gradient-to-r from-blue-400/20 to-indigo-400/20 border border-blue-400/30 text-blue-400 text-xs font-medium py-2.5 px-3 rounded-xl hover:from-blue-400/30 hover:to-indigo-400/30 hover:border-blue-400/40 transition-all duration-300 group/btn"
              @click="openAnalyticsModal(campaign)"
            >
              <div
                class="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent opacity-0 group-hover/btn:opacity-100 -translate-x-full group-hover/btn:translate-x-full transition-all duration-700"
              />
            ...
            <button
              class="flex-1 relative overflow-hidden bg-gradient-to-r from-green-400/20 to-emerald-400/20 border border-green-400/30 text-green-400 text-xs font-medium py-2.5 px-3 rounded-xl hover:from-green-400/30 hover:to-emerald-400/30 hover:border-green-400/40 transition-all duration-300 group/btn"
            ...
            <button
              class="relative overflow-hidden bg-gradient-to-r from-red-400/20 to-rose-400/20 border border-red-400/30 text-red-400 text-xs font-medium py-2.5 px-3 rounded-xl hover:from-red-400/30 hover:to-rose-400/30 hover:border-red-400/40 transition-all duration-300 group/btn"
