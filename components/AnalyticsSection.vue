  ...
  <div class="space-y-8">
          ...
          <div class="relative">
            <div class="flex items-center justify-between mb-4">
              <div>
                ...
                <p class="text-3xl font-bold text-white">{{ filteredAnalytics?.totalScans || 0 }}</p>
              </div>
            ...
            <div class="flex items-center">
                            <span :class="[
                                ...
                                <svg v-if="(growthMetrics?.scansGrowth ?? 0) >= 0" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                ...
                                {{ (growthMetrics?.scansGrowth ?? 0) >= 0 ? '+' : '' }}{{ growthMetrics?.scansGrowth ?? 0 }}%
                            </span>
          ...
          <div class="relative">
            <div class="flex items-center justify-between mb-4">
              <div>
                ...
                <p class="text-3xl font-bold text-white">{{ filteredAnalytics?.uniqueVisitors || 0 }}</p>
              </div>
            ...
            <div class="flex items-center">
                            <span :class="[
                                ...
                                <svg v-if="(growthMetrics?.visitorsGrowth ?? 0) >= 0" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <path d="m5 12 7-7 7 7"/>
                                    <path d="M12 19V5"/>
                                </svg>
                                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                ...
                                {{ (growthMetrics?.visitorsGrowth ?? 0) >= 0 ? '+' : '' }}{{ growthMetrics?.visitorsGrowth ?? 0 }}%
                            </span>
          ...
          <div class="relative">
            <div class="flex items-center justify-between mb-4">
              <div>
                ...
                <p class="text-3xl font-bold text-white">{{ growthMetrics?.conversionRate || 0 }}%</p>
              </div>
            ...
            <div class="flex items-center">
              <span class="text-sm font-medium text-blue-400">Visiteurs uniques / Scans totaux</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="border-b border-white/10">
      <nav class="-mb-px flex space-x-8">
        <button
            v-for="tab in [
                        { value: 'overview', label: 'Vue d\'ensemble', icon: 'home' },
                        { value: 'devices', label: 'Appareils & Navigateurs', icon: 'device' },
                        { value: 'locations', label: 'Géolocalisation', icon: 'map' },
                        { value: 'activity', label: 'Activité Récente', icon: 'activity' }
                    ]"
            :key="tab.value"
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <template v-else-if="tab.icon === 'device'">
              <rect width="16" height="20" x="4" y="2" rx="2" ry="2"/>
              <path d="M9 22v4"/>
              <path d="M15 22v4"/>
            </template>
            <template v-else-if="tab.icon === 'map'">
              <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/>
              <circle cx="12" cy="10" r="3"/>
            </template>
            <template v-else-if="tab.icon === 'activity'">
    ...
    <div v-if="isLoadingAnalytics" class="flex flex-col items-center justify-center py-20">
      <div class="w-16 h-16 bg-blue-400/10 border border-blue-400/20 rounded-2xl flex items-center justify-center mb-6">
    ...
    <div v-else-if="selectedTab === 'overview'" class="space-y-8">
      <div class="grid grid-cols-1 /*lg:grid-cols-2*/ gap-8">
        <div class="p-6 rounded-xl bg-gradient-to-br from-white/[0.03] via-white/[0.05] to-white/[0.03] backdrop-blur-md border border-white/10">
          ...
          <div class="space-y-4">
            <div v-for="(qr, index) in qrcodes.slice().sort((a, b) => (b.scan_count || 0) - (a.scan_count || 0)).slice(0, 5)" :key="qr.id" class="flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
              <div class="flex items-center space-x-4">
                ...
                <div class="w-10 h-10 bg-blue-400/10 border border-blue-400/20 rounded-lg flex items-center justify-center">
          ...
          <div v-if="scansByDay.length > 0" class="relative">
            <div class="flex h-64">
              <div class="w-12 mr-4 relative">
                <div v-for="scale in yAxisScale" :key="scale.value" class="absolute right-0 text-xs text-white/40 font-medium" :style="{ bottom: scale.percentage + '%', transform: 'translateY(50%)' }">
                  {{ scale.value }}
                </div>
              </div>
              ...
              <div class="flex-1 relative">
                <div class="absolute inset-0">
                  <div v-for="scale in yAxisScale" :key="'line-' + scale.value" class="absolute w-full border-t border-white/10" :style="{ bottom: scale.percentage + '%' }"></div>
                </div>

                <div class="h-full flex items-end justify-between space-x-1 relative z-10">
                  <div v-for="(dayData, index) in scansByDay" :key="index" class="flex-1 flex flex-col items-center group">
                    <div class="w-full bg-gradient-to-t from-blue-400 to-blue-300 rounded-t-lg transition-all duration-300 hover:from-blue-300 hover:to-blue-200 relative group-hover:shadow-lg group-hover:shadow-blue-400/25" :style="{
                                            height: dayData.scans > 0 ? Math.max((dayData.scans / yAxisScale[0].value) * 240, 4) + 'px' : '4px'
                                        }">
                      <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 border border-white/20">
                        <div class="font-medium">{{ dayData.scans }} scan{{ dayData.scans > 1 ? 's' : '' }}</div>
    ...
    <div v-else-if="selectedTab === 'devices'" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div class="p-6 rounded-xl bg-gradient-to-br from-white/[0.03] via-white/[0.05] to-white/[0.03] backdrop-blur-md border border-white/10">
        ...
        <div class="space-y-4">
          <div v-if="filteredAnalytics?.deviceBreakdown" class="space-y-4">
            <div v-for="[device, count] in Object.entries(filteredAnalytics.deviceBreakdown)" :key="device" class="flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
      ...
      <div class="p-6 rounded-xl bg-gradient-to-br from-white/[0.03] via-white/[0.05] to-white/[0.03] backdrop-blur-md border border-white/10">
        <h3 class="text-lg font-semibold text-white mb-6 flex items-center gap-3">
          <div class="w-8 h-8 bg-orange-400/10 border border-orange-400/20 rounded-lg flex items-center justify-center">
        ...
        <div class="space-y-4">
          <div v-if="filteredAnalytics?.browserBreakdown" class="space-y-4">
            <div v-for="[browser, count] in Object.entries(filteredAnalytics.browserBreakdown)" :key="browser" class="flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
              <div class="flex items-center space-x-3">
                <div class="w-4 h-4 rounded-full" :class="{
      ...
      <div class="p-6 rounded-xl bg-gradient-to-br from-white/[0.03] via-white/[0.05] to-white/[0.03] backdrop-blur-md border border-white/10">
        ...
        <div class="space-y-4">
          <div v-if="filteredAnalytics?.topCountries?.length" class="space-y-4">
            <div v-for="[country, count] in filteredAnalytics.topCountries" :key="country" class="flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
              <div class="flex items-center space-x-4">
                <div class="w-10 h-10 bg-green-400/10 border border-green-400/20 rounded-lg flex items-center justify-center">
      ...
      <div class="p-6 rounded-xl bg-gradient-to-br from-white/[0.03] via-white/[0.05] to-white/[0.03] backdrop-blur-md border border-white/10">
        ...
        <div class="space-y-4">
          <div v-if="filteredAnalytics?.topCities?.length" class="space-y-4">
            <div v-for="[city, count] in filteredAnalytics.topCities" :key="city" class="flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
              <div class="flex items-center space-x-4">
                <div class="w-10 h-10 bg-purple-400/10 border border-purple-400/20 rounded-lg flex items-center justify-center">
    ...
    <div v-else-if="selectedTab === 'activity'" class="p-6 rounded-xl bg-gradient-to-br from-white/[0.03] via-white/[0.05] to-white/[0.03] backdrop-blur-md border border-white/10">
      <h3 class="text-lg font-semibold text-white mb-6 flex items-center gap-3">
        <div class="w-8 h-8 bg-indigo-400/10 border border-indigo-400/20 rounded-lg flex items-center justify-center">
      ...
      <div class="space-y-4">
        <div v-if="filteredAnalytics?.recentActivity?.length" class="space-y-4 max-h-96 overflow-y-auto">
          <div v-for="activity in filteredAnalytics.recentActivity.slice(0, 10)" :key="activity.id" class="group flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-200">
            <div class="flex items-center space-x-4">
              <div class="w-12 h-12 bg-blue-400/10 border border-blue-400/20 rounded-lg flex items-center justify-center group-hover:bg-blue-400/20 transition-colors">
              ...
              <div>
                <p class="font-medium text-white group-hover:text-blue-400 transition-colors">{{ activity.qrcodes?.name || 'QR Code' }}</p>
            ...
            <div class="text-right">
              <div class="text-sm text-white/80 font-medium">
                {{ new Date(activity.scanned_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) }}
              </div>
              <div class="text-xs text-white/50">
                {{ new Date(activity.scanned_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' }) }}
              </div>
            </div>
          </div>
        </div>
        <div v-else class="flex flex-col items-center justify-center py-20">
          ...
          <p class="text-white/60 text-center max-w-md">Les scans de QR codes apparaîtront ici en temps réel une fois que vos codes commenceront à être utilisés.</p>
        </div>
      </div>
    </div>
  </div>
