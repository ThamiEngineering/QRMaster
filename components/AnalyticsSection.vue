  ...
  <div class="space-y-8">
      ...
      <div class="lg:w-80">
        <div
          ...
          <div class="grid grid-cols-2 gap-2">
            <button v-for="period in ['7d', '30d', '90d', 'all']" :key="period" :class="[
          ...
          <div class="relative">
            ...
            <div class="flex items-center">
              <span :class="[
                ...
                <svg v-if="(growthMetrics?.scansGrowth ?? 0) >= 0" xmlns="http://www.w3.org/2000/svg" width="14"
          ...
          <div class="relative">
            ...
            <div class="flex items-center">
              <span :class="[
                ...
                <svg v-if="(growthMetrics?.visitorsGrowth ?? 0) >= 0" xmlns="http://www.w3.org/2000/svg" width="14"
                  height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                  stroke-linecap="round" stroke-linejoin="round">
                  <path d="m5 12 7-7 7 7" />
                  <path d="M12 19V5" />
    ...
    <div class="border-b border-white/10">
      <nav class="-mb-px flex space-x-8">
        <button v-for="tab in ['overview', 'devices', 'locations', 'activity']" :key="tab" :class="[
          ...
          {{ tab === 'overview' ? "Vue d'ensemble" : tab === 'devices' ? 'Appareils & Navigateurs' : tab === 'locations'
            ?
            'Géolocalisation' : 'Activité Récente' }}
        </button>
      </nav>
    </div>
    ...
    <div v-if="isLoadingAnalytics" class="flex flex-col items-center justify-center py-20">
      <div class="w-16 h-16 bg-blue-400/10 border border-blue-400/20 rounded-2xl flex items-center justify-center mb-6">
        <svg class="animate-spin w-8 h-8 text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none"
          viewBox="0 0 24 24">
              ...
              <div class="flex-1 relative">
                <div class="absolute inset-0">
                  <div v-for="scale in yAxisScale" :key="'line-' + scale.value"
                    class="absolute w-full border-t border-white/10" :style="{ bottom: scale.percentage + '%' }" />
                </div>
      ...
      <div
        ...
        <div class="space-y-4">
          <div v-if="filteredAnalytics?.browserBreakdown" class="space-y-4">
            <div v-for="[browser, count] in Object.entries(filteredAnalytics.browserBreakdown)" :key="browser"
              class="flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
              <div class="flex items-center space-x-3">
                <div class="w-4 h-4 rounded-full" :class="{
    ...
    <div v-else-if="selectedTab === 'activity'"
      ...
      <div class="space-y-4">
        <div v-if="filteredAnalytics?.recentActivity?.length" class="space-y-4 max-h-96 overflow-y-auto">
          <div v-for="activity in filteredAnalytics.recentActivity.slice(0, 10)" :key="activity.id"
