...
const growthMetrics = computed(() => {
    ...
    return {
        ...
        conversionRate: (filteredAnalytics.value.uniqueVisitors / Math.max(currentScans, 1) * 100).toFixed(1)
    }
})
...
const yAxisScale = computed(() => {
    ...
    let maxScale = scales.find(scale => scale >= max) || Math.ceil(max / 100) * 100
    ...
    <div>
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex items-center space-x-2">
                <select v-model="selectedPeriod" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">
                            {{filteredAnalytics?.totalScans || qrcodes.reduce((sum, qr) => sum + (qr.scan_count || 0),
                                0)}}
                        </p>
                    </div>
                ...
                <div class="mt-4 flex items-center">
                    <span
                        :class="['text-sm font-medium', (growthMetrics?.scansGrowth ?? 0) >= 0 ? 'text-green-600' : 'text-red-600']">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ filteredAnalytics?.uniqueVisitors || 0 }}</p>
                    </div>
                ...
                <div class="mt-4 flex items-center">
                    <span
                        :class="['text-sm font-medium', (growthMetrics?.visitorsGrowth ?? 0) >= 0 ? 'text-green-600' : 'text-red-600']">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ growthMetrics?.conversionRate || 0 }}%</p>
                    </div>
        ...
        <div class="mb-6">
            <div class="border-b border-gray-200">
                <nav class="-mb-px flex space-x-8">
                    <button @click="selectedTab = 'overview'" :class="[
                    ...
                    <button @click="selectedTab = 'devices'" :class="[
                        'py-2 px-1 border-b-2 font-medium text-sm',
                        selectedTab === 'devices'
                            ? 'border-blue-500 text-blue-600'
                            : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                    ...
                    <button @click="selectedTab = 'locations'" :class="[
                        'py-2 px-1 border-b-2 font-medium text-sm',
                    ...
                    <button @click="selectedTab = 'activity'" :class="[
                        'py-2 px-1 border-b-2 font-medium text-sm',
                    ...
                    <div v-for="campaign in campaigns" :key="campaign.id"
                        ...
                        <div class="text-right">
                            <p class="font-semibold text-gray-900">
                                {{qrcodes.filter(qr => qr.campaign_id === campaign.id).reduce((sum, qr) => sum +
                                    (qr.scan_count || 0), 0)}}
                            </p>
                            <p class="text-sm text-gray-500">scans</p>
                        </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    <div v-if="filteredAnalytics?.deviceBreakdown" class="space-y-3">
                        <div v-for="[device, count] in Object.entries(filteredAnalytics.deviceBreakdown)" :key="device"
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <div class="w-20 h-2 bg-gray-200 rounded-full">
                                    <div class="h-2 rounded-full" :class="{
                                        ...
                                        :style="{ width: ((count as number) / filteredAnalytics.totalScans * 100) + '%' }">
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    <div v-if="filteredAnalytics?.browserBreakdown" class="space-y-3">
                        <div v-for="[browser, count] in Object.entries(filteredAnalytics.browserBreakdown)"
                            :key="browser" class="flex items-center justify-between">
                            <div class="flex items-center space-x-2">
                                <div class="w-4 h-4 rounded-full" :class="{
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    <div v-if="filteredAnalytics?.topCountries?.length" class="space-y-3">
                        <div v-for="[country, count] in filteredAnalytics.topCountries" :key="country"
                            class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <div class="flex items-center space-x-3">
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-sm text-gray-500">{{ (count / filteredAnalytics.totalScans *
                                    100).toFixed(1) }}%</p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    <div v-if="filteredAnalytics?.topCities?.length" class="space-y-3">
                        <div v-for="[city, count] in filteredAnalytics.topCities" :key="city"
                            class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <div class="flex items-center space-x-3">
        ...
        <div v-else-if="selectedTab === 'activity'" class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div class="space-y-4">
                <div v-if="filteredAnalytics?.recentActivity?.length" class="space-y-4">
                    <div v-for="activity in filteredAnalytics.recentActivity" :key="activity.id"
        ...
        <div v-if="selectedTab === 'overview'" class="bg-white rounded-xl border border-gray-200 p-6 mb-8">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-900">Évolution des scans ({{
            ...
            <div v-if="scansByDay.length > 0" class="relative">
                <div class="flex h-64">
                    <div class="w-14 mr-4 relative">
                        <div v-for="scale in yAxisScale" :key="scale.value"
                    ...
                    <div class="flex-1 relative">
                        <div class="absolute inset-0">
                            <div v-for="scale in yAxisScale" :key="'line-' + scale.value"
                        ...
                        <div class="h-full flex items-end justify-between space-x-1 relative z-10">
                            <div v-for="(dayData, index) in scansByDay" :key="index"
                                class="flex-1 flex flex-col items-center group">
                                <div class="w-full bg-blue-200 rounded-t-lg transition-all duration-300 hover:bg-blue-300 relative group-hover:bg-blue-400"
                                    :style="{
                ...
                <div class="flex mt-2 ml-14">
                    <div v-for="(dayData, index) in scansByDay" :key="'label-' + index"
                        class="flex-1 flex flex-col items-center space-y-1">
                        <p class="text-xs text-gray-500 text-center">{{ dayData.label }}</p>

                        <p v-if="dayData.scans > 0" class="text-xs text-blue-600 font-medium">{{ dayData.scans }}</p>
                    </div>
                </div>
            </div>
            ...
            <div v-else class="h-64 flex items-center justify-center text-gray-500">
                <div class="text-center">
                    ...
                    <p class="text-sm mt-2">Les scans apparaîtront ici une fois que vos QR codes seront utilisés</p>
                </div>
            </div>
        </div>
    </div>
