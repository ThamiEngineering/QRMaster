...
export const useQRTracking = () => {
  ...
  const getQRCodeAnalytics = async (qrcodeId: number) => {
    try {
      ...
      typedData.forEach((scan: QRScanWithQRCode) => {
        if (scan.device_type) {
          analytics.scansByDevice[scan.device_type] =
            (analytics.scansByDevice[scan.device_type] || 0) + 1
        }
      })
  ...
  const getGlobalAnalytics = async (userId: string) => {
    try {
      ...
      const analytics: GlobalAnalytics = {
        totalScans: typedData.length,
        uniqueVisitors: new Set(typedData.map((scan: QRScanWithQRCodeFull) => scan.ip_address))
          .size,
        topCountries: [],
        topCities: [],
        deviceBreakdown: {},
      ...
      typedData.forEach((scan: QRScanWithQRCodeFull) => {
        ...
        if (scan.device_type) {
          analytics.deviceBreakdown[scan.device_type] =
            (analytics.deviceBreakdown[scan.device_type] || 0) + 1
        }

        if (scan.browser) {
          analytics.browserBreakdown[scan.browser] =
            (analytics.browserBreakdown[scan.browser] || 0) + 1
        }
